...
add_action('admin_enqueue_scripts', array(Carbon_Container::class, 'admin_hook_scripts'));
add_action('admin_enqueue_scripts', array(Carbon_Container::class, 'admin_hook_styles'));
...
abstract class Carbon_Container {
	...
	static function factory($type, $name) {
		...
		$class = 'Mpfy\Carbon\Carbon_Container_' . $type;
	...
	function add_fields($fields) {
		foreach ($fields as $field) {
			if ( !is_a($field, 'Mpfy\Carbon\Carbon_Field') ) {
	...
	static function admin_hook_scripts() {
		wp_enqueue_script('mpfy_carbon_containers', MPFY_CARBON_PLUGIN_URL . '/js/containers.js', array('jquery'), '0.5.0');
		wp_localize_script('mpfy_carbon_containers', 'mpfy_carbon_containers_l10n',
			array(
	...
	static function admin_hook_styles() {
		wp_enqueue_style('mpfy_carbon_containers', MPFY_CARBON_PLUGIN_URL . '/css/containers.css', array(), '0.5.0');
	}
