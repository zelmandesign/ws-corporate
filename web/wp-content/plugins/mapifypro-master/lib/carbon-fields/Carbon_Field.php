...
add_action('admin_enqueue_scripts', array(Carbon_Field::class, 'admin_hook_scripts'));
add_action('admin_enqueue_scripts', array(Carbon_Field::class, 'admin_hook_styles'));
add_action('wp_ajax_carbon_relationship_load_posts', array(Carbon_Field_Relationship::class, 'carbon_relationship_load_posts'));

if ( !class_exists('Mpfy\Carbon\Carbon_Field') ) :
...
class Carbon_Field {
	...
	static function factory($type, $name, $label=null) {
		...
		$class = 'Mpfy\Carbon\Carbon_Field_' . $type;
	...
	private function __construct($name, $label) {
		...
		$this->id = 'mpfy-carbon-' . $random_string;
	...
	private function clean_type($type) {
		$clean_class = str_replace('Mpfy\\Carbon\\', '', $type);
		$clean_class = str_replace('Carbon_Field', '', $clean_class);
	...
	static function admin_hook_scripts() {
		wp_enqueue_media();
		wp_enqueue_script('mpfy_carbon_fields', MPFY_CARBON_PLUGIN_URL . '/js/fields.js', array('jquery', 'farbtastic', 'jquery-ui-datepicker'), '0.5.0');
		wp_localize_script( 'mpfy_carbon_fields', 'crbl10n',
			array(
				'title' => __('Files', 'crb'),
	...
	static function admin_hook_styles() {
		wp_enqueue_style('mpfy_carbon_fields', MPFY_CARBON_PLUGIN_URL . '/css/fields.css', array(), '0.5.1');
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Text') ) :

class Carbon_Field_Text extends Carbon_Field {
	function render() {
		echo '<input id="' . $this->get_id() . '" type="text" name="' . $this->name . '" value="' . esc_attr($this->value) . '" class="regular-text" ' . ($this->required ? 'data-mpfy-carbon-required="true"': '') . ' />';
	}
}
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Textarea') ) :
...
class Carbon_Field_Textarea extends Carbon_Field {
	...
	function render() {
		echo '<textarea id="' . $this->get_id() . '" name="' . $this->get_name() . '" style="height: ' . $this->height . 'px; " ' . ($this->required ? 'data-mpfy-carbon-required="true"': '') . '>';
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Rich_Text') ) :
...
class Carbon_Field_Rich_Text extends Carbon_Field_Textarea {
	...
	function render() {
		...
		?>
		<div id="wp-<?php echo $id; ?>-wrap" class="mpfy-carbon-wysiwyg wp-editor-wrap tmce-active" data-toolbar="full">
			...
			<div id="wp-<?php echo $id; ?>-editor-container" class="wp-editor-container">
				<textarea id="<?php echo $id; ?>" class="wp-editor-area" name="<?php echo $this->get_name(); ?>" <?php echo ($this->required ? 'data-mpfy-carbon-required="true"': '') ?>><?php echo wp_richedit_pre($val); ?></textarea>
			</div>
		</div>
		<?php
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Date') ) :
...
class Carbon_Field_Date extends Carbon_Field {
	function init() {
		...
		if (defined('WP_ADMIN') && WP_ADMIN) {
			...
			wp_enqueue_style('mpfy-carbon-jquery-ui', MPFY_CARBON_PLUGIN_URL . '/css/jquery-ui.css');
		}
		Carbon_Field::init();
	}

	function render() {
		echo '<input id="' . $this->get_id() . '" type="text" name="' . $this->get_name() . '" value="' . esc_attr($this->value) . '" class="regular-text mpfy-carbon-datepicker"  ' . ($this->required ? 'data-mpfy-carbon-required="true"': '') . '/>';
		echo '<span class="mpfy-mpfy-carbon-datepicker-trigger button mpfy-icon-button hide-if-no-js">' . __('Date', 'crb') . '</span>';
	}
}
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Color') ) :
...
class Carbon_Field_Color extends Carbon_Field {
	...
	function render() {
		echo '
		<div class="mpfy-carbon-color-row">
			<input id="' . $this->get_id() . '" type="text" name="' . $this->get_name() . '" value="' . esc_attr($this->value) . '" class="regular-text mpfy-carbon-color" ' . ($this->required ? 'data-mpfy-carbon-required="true"': '') . ' />
			<a class="mpfy-carbon-color-preview hide-if-no-js"></a>
			<span class="pickcolor button mpfy-icon-button hide-if-no-js">' . __('Select a Color', 'crb') . '</span>
			<div class="mpfy-carbon-color-container hide-if-no-js"></div>
		</div>';
	}
}
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Map') ) :
...
class Carbon_Field_Map extends Carbon_Field {
	...
	function admin_init() {
		wp_enqueue_script('mpfy-carbon-google-maps', 'http://maps.googleapis.com/maps/api/js?sensor=false');
	}

	function render() {
		echo '
		<input type="text" name="' . $this->get_name() . '" value="' . esc_attr($this->value) . '" class="regular-text mpfy-carbon-map-field" data-zoom="' . esc_attr($this->zoom) . '" data-default-lat="' . esc_attr($this->default_lat) . '" data-default-lng="' . esc_attr($this->default_lng) . '"  ' . ($this->required ? 'data-mpfy-carbon-required="true"': '') . '/>
		<div class="mpfy-carbon-map-canvas">&nbsp;</div>
		';
	}
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Map_With_Address') ) :

class Carbon_Field_Map_With_Address extends Carbon_Field_Map {
	protected $address = '';

	function render() {
		echo '<p>' . __('Locate Address on the map', 'crb') . ': <input type="text" name="' . esc_attr($this->get_name()) . '[address]" value="' . esc_attr($this->address) . '" class="regular-text address" /><span class="address-search-btn button mpfy-icon-button">' . __('Find', 'crb') . '</span></p>';

		echo '
		<input type="text" name="' . $this->get_name() . '[coordinates]" value="' . esc_attr($this->value) . '" class="regular-text mpfy-carbon-map-field" data-zoom="' . esc_attr($this->zoom) . '" data-default-lat="' . esc_attr($this->default_lat) . '" data-default-lng="' . esc_attr($this->default_lng) . '"  ' . ($this->required ? 'data-mpfy-carbon-required="true"': '') . '/>
		<div class="mpfy-carbon-map-canvas">&nbsp;</div>
		';
	}
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Select') ) :
...
class Carbon_Field_Select extends Carbon_Field {
	...
	function render() {
		...
		echo '<select id="' . $this->get_id() . '" name="' . $this->get_name() . '" ' . ($this->required ? 'data-mpfy-carbon-required="true"': '') . '>';
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Radio') ) :
...
class Carbon_Field_Radio extends Carbon_Field {
	...
	function render() {
		...
		echo '<ul class="mpfy-carbon-radio-list" ' . ($this->required ? 'data-mpfy-carbon-required="true"': '') . '>';
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Checkbox') ) :
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Header_Scripts') ) :
...
class Carbon_Field_Header_Scripts extends Carbon_Field_Textarea {
	...
	function print_scripts() {
		if ( !$this->store || !is_a($this->store, 'Mpfy\Carbon\Carbon_DataStore_ThemeOptions') ) {
			return;
		}
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Footer_Scripts') ) :
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Separator') ) :
...
class Carbon_Field_Separator extends Carbon_Field {
	...
	function get_label() {
		...
		return '<h3 class="title mpfy-carbon-separator">' . $label . '</h3>';
	}
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Set') ) :
...
class Carbon_Field_Set extends Carbon_Field {
	...
	function render() {
		...
		echo '<div class="mpfy-carbon-set-list" ' . ($this->required ? 'data-mpfy-carbon-required="true"': '') . '>';
		...
		foreach ($this->options as $key => $value) {
			...
			if ( $this->limit_options > 0 && $loopCount > $this->limit_options ) {
				...
				if ( $loopCount == $this->limit_options ) {
					echo '<p>... <a href="#" class="mpfy-carbon-set-showall">' . __('Show All Options', 'crb') . '</a></p>';
				}
			}
		}
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Relationship') ) :
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_File') ) :
...
class Carbon_Field_File extends Carbon_Field {
	...
	function render() {
		echo '<input
				...
				class="regular-text mpfy-carbon-file-field" ' . ($this->required ? 'data-mpfy-carbon-required="true"': '') . '
				'.($this->value_type=='id' ? 'style="display:none"' : '').'
			/>';

		echo '<span
				id="c2_open_media' . str_replace('-', '_', $this->id) .  '"
				class="button c2_open_media mpfy-icon-button"
	...
	function description() {
		$description = '<a href="' . $this->value . '" target="_blank" class="mpfy-carbon-view_file" style="' . (!empty($this->value) ? '' : 'display:none;' ) . '" >' . __('View File', 'crb') . '</a>';

		return apply_filters('mpfy_carbon_field_' . $this->type . '_description', '<div class="mpfy-carbon-description">' . $description . '</div>');
	}
}
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Attachment') ) :
...
class Carbon_Field_Attachment extends Carbon_Field_File {
	...
	function description(){
		...
		if ( $is_image ) {
			$description .= '<div class="mpfy-carbon-preview">';
			$description .= '<img src="' . $thumbnail_src[0] . '" alt="" class="mpfy-carbon-view_image"/>';
			$description .= '<span class="mpfy-carbon-file-remove mpfy-icon-button"></span>';
			$description .= '</div>';
		} else {
			$description .= '<div class="mpfy-carbon-preview">';
			$description .= '<img src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" alt="" class="mpfy-carbon-view_image blank"/>';
			$description .= '</div>';
		}
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Image') ) :
...
class Carbon_Field_Image extends Carbon_Field_File {
	...
	function description(){
		...
		if ( $this->value != '' ) {
			$description .= '<div class="mpfy-carbon-preview">';
			$description .= '<img src="' . $this->value . '" alt="" class="mpfy-carbon-view_image"/>';
			$description .= '<span class="mpfy-carbon-file-remove"></span>';
			$description .= '</div>';
		} else {
			$description .= '<div class="mpfy-carbon-preview">';
			$description .= '<img src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" alt="" class="mpfy-carbon-view_image blank"/>';
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Choose_Sidebar') ) :
...
class Carbon_Field_Choose_Sidebar extends Carbon_Field_Select {
	...
	function init() {
		...
		add_action('mpfy_carbon_after_register_fields', array($this, 'setup_sidebar_options'), 20);
		add_action('mpfy_carbon_after_register_fields', array($this, 'register_custom_sidebars'), 21);
	}
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_HTML') ) :
...
if ( !class_exists('Mpfy\Carbon\Carbon_Field_Gravity_Form') ) :
...
class Carbon_Field_Gravity_Form extends Carbon_Field_Select {
	function init() {
		// Setup Form Options
		add_action( 'mpfy_carbon_after_register_fields', array($this, 'setup_gravity_form_options'), 20 );
	}
	...
	function render() {
		...
		$this->set_options( apply_filters( 'mpfy_crb_gravity_form_options', $this->options ) );
