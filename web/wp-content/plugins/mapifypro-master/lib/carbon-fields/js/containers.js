jQuery(function($) {
	function init (context) {
		...
		containers = $('.mpfy-carbon-container', context);
	...
	function check_required() {
		...
		$('.mpfy-carbon-highlight').removeClass('mpfy-carbon-highlight');

		// Find required fields
		$('input[type="text"][data-mpfy-carbon-required=true], textarea[data-mpfy-carbon-required=true]').each(function() {
			var th = $(this);

			if ( th.val() != '' || th.closest('.mpfy-carbon-group-preview, .mpfy-carbon-skip-validation').length > 0 ) {
				return;
			};

			th.closest('td').addClass('mpfy-carbon-highlight');
			has_errors = true;
		})
		$('.mpfy-carbon-radio-list[data-mpfy-carbon-required=true], .mpfy-carbon-set-list[data-mpfy-carbon-required=true]').each(function() {
			var th = $(this);

			if ( th.find('input[type=radio]:checked, input[type=checkbox]:checked').length > 0 || th.closest('.mpfy-carbon-group-preview, .mpfy-carbon-skip-validation').length > 0 ) {
				return;
			};
	...
	function theme_options_attach_validation_hook() {
		...
		$('.Carbon_Container_ThemeOptions:first > form').live("submit", function(){
			...
			form.siblings('.mpfy-carbon-error-required').remove();
			form.before($('<div class="settings-error error below-h2 mpfy-carbon-error-required"><p><strong>' + mpfy_carbon_containers_l10n.please_fill_the_required_fields + '</strong></p></div>'));
		...
		setTimeout(function() {
			...
			window.onbeforeunload = function (){
				if ( carbon_container.ThemeOptions.hasChanges ) {
					return (typeof autosaveL10n != 'undefined' ? autosaveL10n.saveAlert: mpfy_carbon_containers_l10n.changes_made_save_alert);
				};
			...
			$('.mpfy-carbon-container .mpfy-carbon-field input, .mpfy-carbon-container .mpfy-carbon-field select, .mpfy-carbon-container .mpfy-carbon-field textarea').live('change', function() {
			...
			$('.mpfy-carbon-container input[type="submit"]').click(function(){
			...
			if ( old_callback == null ) {
				$(window).on( 'beforeunload.edit-post', function (){
					if ( carbon_container.CustomFields.hasChanges ) {
						return (postL10n && postL10n.saveAlert ? postL10n.saveAlert: mpfy_carbon_containers_l10n.changes_made_save_alert);
					};
				})
			} else {
				window.onbeforeunload = function (){
					if ( carbon_container.CustomFields.hasChanges ) {
						return (autosaveL10n && autosaveL10n.saveAlert ? autosaveL10n.saveAlert: mpfy_carbon_containers_l10n.changes_made_save_alert);
					};
	...
	function custom_fields_check_visible(container) {
		...
		container.node.toggleClass('mpfy-carbon-skip-validation', !show);
	...
	carbon_container.Widget = function (element, container_obj) {
		...
		if (is_template_widget) {
			element.find('.mpfy-carbon-field').addClass('mpfy-carbon-field-skip');
		} else {
			element.find('.mpfy-carbon-field').removeClass('mpfy-carbon-field-skip');
	...
	function widget_check_required(context) {
		...
		$('.mpfy-carbon-highlight', context).removeClass('mpfy-carbon-highlight');

		// Find required fields
		$('input[type="text"][data-mpfy-carbon-required=true], textarea[data-mpfy-carbon-required=true]', context).each(function() {
			var th = $(this);
			...
			th.closest('.mpfy-carbon-widget-field-wrapper').addClass('mpfy-carbon-highlight');
			has_errors = true;
		})
		$('.mpfy-carbon-radio-list[data-mpfy-carbon-required=true], .mpfy-carbon-set-list[data-mpfy-carbon-required=true]', context).each(function() {
			var th = $(this);
	...
	function widget_attach_save_alert(container) {
		...
		setTimeout(function () {
			...
			window.onbeforeunload = function (){
				var response = (typeof autosaveL10n != 'undefined' ? autosaveL10n.saveAlert: mpfy_carbon_containers_l10n.changes_made_save_alert);
				var custom_carbon_widgets_has_errors = false;
				var $custom_carbon_widgets = $('.widget-liquid-right .mpfy-carbon-container', document).closest('form');
			...
			$('.mpfy-carbon-field input, .mpfy-carbon-field select, .mpfy-carbon-field textarea', container).on('change', function() {
	...
	function widget_attach_validation_hook(container) {
		...
		$carbon_widget_container_form.find('input[type="submit"]').on('click', function() {
			...
			$('body .wrap').find('> .mpfy-carbon-error-required').remove();
			...
			$('body, html').animate({scrollTop: 0});
			$('body .wrap > h2').after( $('<div class="settings-error error below-h2 mpfy-carbon-error-required"><p><strong>' + mpfy_carbon_containers_l10n.please_fill_the_required_fields + '</strong></p></div>') );
			carbon_container.Widget.attachedSaveAlert = false;
		...
		setTimeout(function() {
			...
			window.onbeforeunload = function (){
				if ( carbon_container.UserMeta.hasChanges ) {
					return (typeof autosaveL10n != 'undefined' && autosaveL10n.saveAlert ? autosaveL10n.saveAlert: mpfy_carbon_containers_l10n.changes_made_save_alert);
				};
	...
	if ( !$('body').is('.mobile') ) {
		$('body').addClass('mpfy-carbon-desktop');
	};
});