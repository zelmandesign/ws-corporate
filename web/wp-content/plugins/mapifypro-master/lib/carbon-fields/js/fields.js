jQuery(function($) {
	...
	function init(context) {
		...
		fields = $('.mpfy-carbon-field:not(.mpfy-carbon-field-skip)', context);
		...
		fields.each(function() {
			...
			if ( typeof mpfy_carbon_field[type] == 'undefined' ) {
				return;
			};

			try {
				field = mpfy_carbon_field(th);
				mpfy_carbon_field[type](th, field);
			} catch (e) {
				carbon_log_error("Couldn't render a field: " + (e.message || e) );
			}
		});
		
		$('table.layout-table em.mpfy-help-text', context).each(function () {
			var fileField = $(this).closest('tr').find('td div.mpfy-carbon-file');
			
			if (fileField.length) {
				$(this).insertBefore(fileField.find('div.mpfy-carbon-description')).wrap('<div class="mpfy-help-text" />');
			}
		});
	}
	...
	function remove_fields(context) {
		...
		$('.mpfy-carbon-field:not(.mpfy-carbon-field-skip)', context).trigger('remove_fields.carbon');
	}

	function mpfy_carbon_field(node) {
		var field = {};

		if ( node.data('mpfy_carbon_field') ) {
			node.trigger('reinit_field.carbon');
			$.error('Field already parsed');
		};

		node.data('mpfy_carbon_field', field);
		field.node = node;
	...
	/*File*/
	mpfy_carbon_field.File = function(element, field_obj) {
		...
		$(element).on('click', '.mpfy-carbon-file-remove', function (e) {
	...
	/* Attachment */
	mpfy_carbon_field.Attachment = mpfy_carbon_field.File;
	
	/* Image */
	mpfy_carbon_field.Image = mpfy_carbon_field.File;

	/* Date picker */
	mpfy_carbon_field.Date = function(element, field_obj) {
		var text_field = element.find('.mpfy-carbon-datepicker');
		...
		element.find('.mpfy-mpfy-carbon-datepicker-trigger').click(function() {
			text_field.focus();
			return false;
		});
	}
	...
	/* Color picker */
	mpfy_carbon_field.Color = function(element, field_obj) {
		var color_container = element.find('.mpfy-carbon-color-container'),
			color_preview = element.find('.mpfy-carbon-color-preview'),
			color_button = element.find('.button'),
			color_field = element.find('.mpfy-carbon-color'),
			farbtastic_obj;
		...
		$('body').click(function(e) {
			...
			if ( $target.closest('.mpfy-carbon-color-container').length > 0 ) {
				return false;
			};
	...
	/* Choose Sidebar */
	mpfy_carbon_field.Choose_Sidebar = function(element, field_obj) {
	...
	/* Map */
	mpfy_carbon_field.Map = function(element, field_obj) {
		var field = element.find('.mpfy-carbon-map-field'),
			map_container = element.find('.mpfy-carbon-map-canvas'),
			exists = 0,
			marker = false,
	...
	/* Map With Address */
	mpfy_carbon_field.Map_With_Address = function(element, field_obj) {
		var search_field = element.find('.address'),
			geocoder = new google.maps.Geocoder();

		// Initialize the base map field
		mpfy_carbon_field.Map(element, field_obj);
	...
	/* Rich text */
	mpfy_carbon_field.Rich_Text = function(element, field_obj) {
		var textarea = element.find('.mpfy-carbon-wysiwyg textarea'),
	...
	/* Relationship */
	mpfy_carbon_field.Relationship = function(element, field_obj) {
		var container = element.find('.mpfy-carbon-relationship'),
	...
	mpfy_carbon_field.Set = function (element, field_obj) {
		element.find('a.mpfy-carbon-set-showall').click(function (e) {
			e.preventDefault();
	...
	/* Complex Field */
	mpfy_carbon_field.Complex = function(element, field_obj) {
		// prepare object
		field_obj.btn_add = element.find('> .mpfy-carbon-subcontainer > tbody > tr.mpfy-carbon-actions a[data-action=add]:first').parent();
		field_obj.group_selector = field_obj.btn_add.find('ul');
		field_obj.empty_field_text = element.find('> .mpfy-carbon-subcontainer > tbody > tr.mpfy-carbon-empty-row');
		field_obj.num_rows = element.find('> .mpfy-carbon-subcontainer > tbody > tr.mpfy-carbon-group-row').length;
		field_obj.row_uid = field_obj.num_rows;
		field_obj.table = element.children('.mpfy-carbon-subcontainer:first');
		field_obj.min_rows = element.children('.mpfy-carbon-subcontainer').data('min-values');
		field_obj.max_rows = element.children('.mpfy-carbon-subcontainer').data('max-values');
		...
		$('tr.mpfy-carbon-group-row[id]').each(function () {
		...
		field_obj.node.find('a[data-action="toggle-minimize"]').live('click', function (e) {
			...
			var groupRow = $(this).closest('.mpfy-carbon-group-row');
		...
		field_obj.node.find('a[data-action="remove"]').live('click', function (e) {
			e.preventDefault();
			e.stopPropagation();
			complex_remove_row(field_obj, $(this).closest('.mpfy-carbon-group-row'));
		});

		field_obj.node.find('a[data-action="duplicate"]').live('click', function (e) {
			e.preventDefault();
			e.stopPropagation();
			complex_duplicate_row(field_obj, $(this).closest('.mpfy-carbon-group-row'));
		});
		...
		field_obj.table.children('tbody').sortable({
			items : '> tr.mpfy-carbon-group-row',
			handle: '.mpfy-carbon-drag-handle',
			forceHelperSize: true,
			forcePlaceholderSize: true,
			scroll: true,
			...
			start: function( ev, ui) {
				field_obj.table.addClass('mpfy-carbon-container-shrank')
				remove_fields(ui.item);
			},
			stop: function( ev, ui) {
				field_obj.table.removeClass('mpfy-carbon-container-shrank')
				init(ui.item);
			}
		});
	}
	...
	function complex_add_row(field) {
		...
		sample_row = field.node.find('> .mpfy-carbon-subcontainer > tbody > .mpfy-carbon-group-preview.mpfy-carbon-group-' + field.new_row_type);
		new_row = sample_row.clone();

		field.num_rows++;
		field.row_uid++;

		new_row.find('.mpfy-carbon-field-skip').removeClass('mpfy-carbon-field-skip');
		...
		new_row.removeClass('mpfy-carbon-group-preview').addClass('mpfy-carbon-group-row').insertBefore( field.node.find('> .mpfy-carbon-subcontainer > tbody > .mpfy-carbon-group-preview:first') );
		init(new_row);

		new_row.find('.mpfy-carbon-drag-handle:first span').text(field.num_rows)
	...
	function complex_duplicate_row(field, row) {
		...
		new_row.removeClass('mpfy-carbon-group-preview').addClass('mpfy-carbon-group-row').insertAfter( row );
		init(new_row);
	...
	function complex_on_update_rows(field) {
		var fields = field.node.find('.ui-sortable:first').children('.mpfy-carbon-group-row'),
			row_index = 0;
		field.num_rows = fields.length;

		for (var i = field.num_rows; i >= 0; i--) {
			fields.eq(i).find('.mpfy-carbon-drag-handle:first span').text(i+1);
		};
	...
	window.mpfy_carbon_field_init = init;
	window.mpfy_carbon_field = mpfy_carbon_field;
