...
<div class="postbox wps-postbox-license-activation">
  ...
  <div class="inside">
    ...
    <form method="post" name="cleanup_options" action="" id="wps-license" class="wps-admin-form">
      ...
      <!-- Nonce -->
      <input hidden type="text" class="regular-text" id="<?= WP_SHOPIFY_SETTINGS_GENERAL_OPTION_NAME; ?>_nonce_license_id" name="<?= WP_SHOPIFY_SETTINGS_GENERAL_OPTION_NAME; ?>[nonce]" value="<?php echo wp_create_nonce( uniqid() ); ?>"/>
      ...
      <div class="wps-form-group">
        ...
        <small class="wps-is-hidden">
          <?php printf(__('You can find your license key <a href="%1$s" target="_blank">within your account</a> or contained inside your payment confirmation email.', WP_SHOPIFY_PLUGIN_TEXT_DOMAIN), esc_url("https://wpshop.io/login")); ?>
        </small>
      ...
      <div class="wps-button-group button-group button-group-ajax wps-is-hidden">
        ...
        <?php if ( $DB_Settings_License->has_license_key($license) ) { ?>
          <?php submit_button(esc_html__('Deactivate License', WP_SHOPIFY_PLUGIN_TEXT_DOMAIN), 'primary', 'submit-license', false, array('data-status' => 'deactivate')); ?>

        <?php } else { ?>
          <?php submit_button(esc_html__('Activate License', WP_SHOPIFY_PLUGIN_TEXT_DOMAIN), 'primary', 'submit-license', false, array('data-status' => 'activate')); ?>
