...
class Collects extends \WP_Shopify\API {
	...
	*/
	public function get_collects_count_by_collection_ids() {
		...
		foreach ($collection_ids as $collection_id) {
         ...
         $count = $this->Shopify_API->sanitize_response($count);
	...
	*/
	public function get_collects_count($request) {
      ...
      return $this->handle_response([
			'response' 				=> $this->Shopify_API->sanitize_response($this->Shopify_API->get_collects_count()),
			'access_prop'			=> 'count',
			'return_key' 			=> 'collects',
			'warning_message'	=> 'collects_count_not_found'
		]);
	...
	public function get_collects_from_collections($current_page) {
		...
		foreach ($collection_ids as $collection_id) {
         ...
         $result = $this->Shopify_API->sanitize_response($this->Shopify_API->get_collects_from_collection_per_page($collection_id, $param_limit, $current_page));
	...
	public function get_collects_per_page($current_page) {
      ...
      return $this->Shopify_API->sanitize_response($this->Shopify_API->get_collects_per_page( $this->DB_Settings_General->get_items_per_request(), $current_page));
	...
	*/
	public function get_collects_from_product($item = null) {
      ...
      $collects = $this->Shopify_API->sanitize_response($this->Shopify_API->get_collects_by_product_id($item->id));
	...
	*/
	public function get_collects_from_collection($item) {
      ...
      $collects = $this->Shopify_API->sanitize_response($this->Shopify_API->get_collects_from_collection_id($item->id));
