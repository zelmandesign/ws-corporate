...
class Transients {
	...
	*/
	...
	public static function delete_all_custom_options() {
		...
		$plugin_options = $wpdb->get_results("SELECT option_name FROM $wpdb->options WHERE option_name LIKE 'wp_shopify_%' OR option_name LIKE 'wps_settings_%' OR option_name LIKE '%_transient_wp_shopify_%'" );
	...
	*/
	public static function delete_long_term_cache() {
      ...
      $query = "DELETE FROM " . $wpdb->options . " WHERE `option_name` LIKE '%wp_shopify_%' AND `option_name` NOT LIKE '%wp_shopify_cache_cleared%'";
