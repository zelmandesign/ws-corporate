...
class Updater {
	...
	public function check_for_updates($license) {
		...
		*/
		...
		if ( !class_exists( 'EDD_SL_Plugin_Updater' ) ) {
			include( WP_SHOPIFY_PLUGIN_DIR_PATH . 'vendor/EDD/EDD_SL_Plugin_Updater.php' );
      }
      
		// Setup the updater
		// Calls the init() function within the constructor
		return new \EDD_SL_Plugin_Updater( EDD_SL_STORE_URL, WP_SHOPIFY_BASENAME, [
	...
	*/
	...
	public function remove_plugin_updates($value) {
		...
		if (is_object($value)) {
			unset( $value->response[WP_SHOPIFY_BASENAME] );
			return $value;
		}
